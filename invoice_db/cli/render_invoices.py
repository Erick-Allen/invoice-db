...
def invoice_not_found(invoice_id: int | None = None) -> None:
    if invoice_id is not None:
        ui.console.print(f"Invoice not found (id={invoice_id})", style ="warning")
    else:
        ui.console.print("Invoice not found", style="warning")

def no_invoices_found() -> None:
    ui.console.print("No invoices found", style="warning")
...
def print_invoice_count(count: int, label: str) -> None:
    if label:
        ui.console.print(f"[accent]Matched invoices: [highlight]{count}[/highlight]")
    else:
        ui.console.print(f"[accent]Total invoices:[/accent] [highlight]{count}[/highlight]")

def print_invoice_update(id: int, fields: str) -> None:
    if fields:
        ui.console.print(f"Updated invoice (id={id}, fields: {fields})", style="success")
